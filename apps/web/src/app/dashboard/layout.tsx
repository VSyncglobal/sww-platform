...
export default function DashboardLayout({
  ...
  const menuItems = [
    ...
    { name: 'Welfare', icon: PiggyBank, href: '/dashboard/welfare' },
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans text-gray-900">
      ...
      {/* SIDEBAR */}
      <aside 
        className={`fixed lg:static inset-y-0 left-0 z-30 w-64 bg-[#1e3a8a] text-white transition-transform duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
      >
        {/* LOGO AREA */}
        ...
        <div className="absolute bottom-0 w-full p-4 border-t border-blue-800">
          <button className="flex items-center gap-3 w-full px-4 py-3 text-red-200 hover:bg-red-900/20 hover:text-red-100 rounded-xl transition">
            <LogOut size={20} />
            Sign Out
          </button>
        </div>
        ...
        <header className="h-20 bg-white border-b border-gray-200 flex items-center justify-between px-8">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden sm:block">
              ...
              <p className="text-xs text-gray-500">Member ID: #8821</p>
            </div>
